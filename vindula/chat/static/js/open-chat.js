...
var URL_CHAT 		= "chat.php?action=chatheartbeat";
var URL_CHAT_close  = "chat.php?action=closechat"
var URL_CHAT_send   = "chat.php?action=sendchat"
var URL_CHAT_start  = "chat.php?action=startchatsession"
...
function restructureChatBoxes() {
	...
	for (x in chatBoxes) {
		...
		if ($j("#chatbox_"+chatboxtitle).css('display') != 'none') {
			if (align == 0) {
				$j("#chatbox_"+chatboxtitle).css('right', '250px');
			} else {
...
function createChatBox(chatboxtitle,minimizeChatBox,conteudo) {
    //conteudo = '';
	if ($j("#chatbox_"+chatboxtitle).length > 0) {
		...
		$j("#chatbox_"+chatboxtitle).find('.chatboxcontent').append(conteudo);
    ...
    var jid = $j("#"+chatboxtitle).attr('data-recipient');
	$j(" <div />" ).attr("id","chatbox_"+chatboxtitle)
                	...
                	.html('<div class="chatboxhead"><div class="chatboxtitle">'+chatboxtitle+'</div>\
                		   ...
                		   <div class="chatboxoptions"><a href="javascript:void(0)" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')">-</a>\
                			<a href="javascript:void(0)" onclick="javascript:closeChatBox(\''+chatboxtitle+'\')">X</a></div><br clear="all"/></div>\
                			<div class="chatboxcontent">'+conteudo+'</div><div class="chatboxinput">\
...
function closeChatBox(chatboxtitle) {
	...
	restructureChatBoxes();
	//$j.post(URL_CHAT_close, { chatbox: chatboxtitle} , function(data){ });
	$j(document).trigger('jarnxmpp.disconnecting');
}
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
        var text = $j(chatboxtextarea).val();
		text = text.replace(/^\s+|\s+$j/g,"");
        $j(chatboxtextarea).val('');
        $j(chatboxtextarea).focus();
        $j(chatboxtextarea).css('height','44px');
        var username = $j('#username').val();
        var to_menssage = $j(chatboxtextarea).parent().parent().find('#jid').val();
        if (text != '') {
            $j.getJSON(portal_url + '/content-transform?', {text: text}, function (data) {
                message = $msg({to: to_menssage, type: 'chat'}).c('body').t(data.text);
                jarnxmpp.connection.send(message);
                
                //message = message.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;");
                $j("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage">\
                                                                        <span class="chatboxmessagefrom">'+username.toUpperCase()+':&nbsp;&nbsp;</span>\
                                                                        <span class="chatboxmessagecontent">'+text+'</span></div>');
                $j("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop($j("#chatbox_"+chatboxtitle+" .chatboxcontent")[0].scrollHeight);
